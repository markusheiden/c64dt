...
public class JC64TextField extends JComponent {
  ...
  private void init() {
    if (charRom == null) {
      try {
        charRom = ResourceLoader.load(0x1000, "/roms/character/display.bin");
      } catch (IOException e) {
        Assert.isTrue(false, "Precondition: Char rom exists and is readable, but got: " + e.getMessage());
      }
    }
  }
  ...
  protected final void drawLine(Graphics g, int x, int y, int image) {
    for (int i = 0; i < 8; i++, x++) {
      if ((image & 0x80) != 0) {
        g.fillRect(x, y, 1, 1);
      }
